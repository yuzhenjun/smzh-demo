...
public class SortUtils {
	...
	public static void arraysSort() {
		...
		System.out.println("arraysSort排序耗时:" + (endTime - beginTime) + "ms");
	}
	...
	public static void forkJoinSort() {
		...
		long endTime = System.currentTimeMillis();
		System.out.println("forkJoinSort排序耗时:" + (endTime - beginTime) + "ms");
	}
	...
	public static void writeRrandomFile() {
		...
		long endTime = System.currentTimeMillis();
		System.out.println("写文件耗时:" + (endTime - beginTime) + "ms");
	}
	...
	public static void readRrandomFile() {
		...
		long endTime = System.currentTimeMillis();
		System.out.println("读文件耗时:" + (endTime - beginTime) + "ms");
	}
	...
	public static void sort() {
		...
		long endTime = System.currentTimeMillis();
		System.out.println("sort排序耗时:" + (endTime - beginTime) + "ms");
	}
	...
	public static void sort(final int a[], final int low, final int high) {
		...
		final int imm = i - 1;
		if (low < imm) {
			if (SortUtils.THREADCOUNT.get() > 0 && imm - low > S) {
				Thread t = new Thread() {
					{
						SortUtils.THREADCOUNT.decrementAndGet();
					}

					@Override
					public void run() {
						sort(a, low, imm);
						SortUtils.THREADCOUNT.addAndGet(1);
					}
				};
				t.start();
				threads.add(t);
		...
		final int ipp = i + 1;
		if (high > ipp) {
			if (SortUtils.THREADCOUNT.get() > 0 && high - ipp > S) {
				Thread t = new Thread() {
					{
						SortUtils.THREADCOUNT.decrementAndGet();
					}
...
class SortTask extends RecursiveAction {
	...
	public static void main(String[] args) throws IOException,InterruptedException {
		//SortUtils.writeRrandomFile();
		SortUtils.readRrandomFile();
		SortUtils.sort();
		//SortUtils.forkJoinSort();
		//checkSort();
		//SortUtils.writeSortFile();
		/*
		 * 读文件耗时:12500ms
           Arrays.sort排序耗时:4680ms
		*/
		/*
		 * 读文件耗时:11719ms
           sort排序耗时:3128ms
